...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        m[j].distance = l1_distance(a[j].data, b[0].data, b[0].n); // <- should be the smallest L1 distance!
        for (i = 1; i < bn; i++) {
            float dist = l1_distance(a[j].data, b[i].data, b[i].n);
            if (dist < m[j].distance) {
                bind = i;
                m[j].distance = dist;
            }
        }
...
image combine_images(image a, image b, matrix H)
{
    ...
    for (i = 0; i < c.w; i++) {
        for (j = 0; j < c.h; j++) {
            point projected = project_point(H, make_point(i, j)); // Is it i,j and j,i?
